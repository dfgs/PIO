...
namespace PIO.UnitTest.ServerLib.Mocks
{
	public class MockedTaskModule :MockedDatabaseModule<Task>,ITaskModule
	{
		...
		public Task CreateTask(TaskTypeIDs TaskTypeID, int WorkerID, int? PlanetID, int? TargetFactoryID, ResourceTypeIDs? ResourceTypeID, FactoryTypeIDs? FactoryTypeID, DateTime ETA)
		{
			...
			item = new Task() { TaskTypeID = TaskTypeID, PlanetID = PlanetID, WorkerID=WorkerID,  TargetFactoryID =TargetFactoryID,ResourceTypeID=ResourceTypeID,FactoryTypeID=FactoryTypeID, ETA=ETA };
			items.Add(item);
			return item;
		}
