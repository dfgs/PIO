...
namespace PIO.UnitTest.ServerLib.Modules
{
	...
	public class ProducerModuleUnitTest
	{
		...
		public void ShouldReturnTaskWhenFactoryHasEnoughResourcesToProduce()
		{
			...
			module = new ProducerModule(NullLogger.Instance,factoryModule,workerModule,stackModule,ingredientModule,productModule, taskModule);
			schedulerModule = new MockedSchedulerModule(false,module);

			result = module.BeginProduce(1, 1);
		...
		public void ShouldReturnNullWhenFactoryHasNoProduct()
		{
			...
			productModule = new MockedProductModule(false, new Product() { ProductID = 1, FactoryTypeID = 22222, Duration = 4, Quantity = 2 });
			taskModule = new MockedTaskModule(false, 1);
			module = new ProducerModule(NullLogger.Instance, factoryModule, workerModule, stackModule, ingredientModule, productModule, taskModule);
			schedulerModule = new MockedSchedulerModule(false, module);
		...
		public void ShouldThrowExceptionAndLogErrorWhenFactoryHasNotEnoughResourcesToProduce()
		{
			...
			Assert.ThrowsException<PIONoResourcesException>(() => module.BeginProduce(1, 1));
		...
		public void ShouldThrowExceptionAndLogErrorWhenWorkerDoesntExists()
		{
			...
			Assert.ThrowsException<PIONotFoundException>(() => module.BeginProduce(2, 1));
			Assert.IsNotNull(logger.Logs.FirstOrDefault(item => item.Contains("Warning") && item.Contains(module.ModuleName)));
			Assert.AreEqual(0, schedulerModule.Count);
		...
		public void ShouldThrowExceptionAndLogErrorWhenSubModuleFails()
		{
			...
			module = new ProducerModule(logger, factoryModule, workerModule,stackModule, ingredientModule, productModule, taskModule);
			schedulerModule = new MockedSchedulerModule(false, module);
			Assert.ThrowsException<PIOInternalErrorException>(() => module.BeginProduce(1,1));
