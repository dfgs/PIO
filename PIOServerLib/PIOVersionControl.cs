		...
		protected override IEnumerable<IQuery> OnUpgradeTo(int Version)
		{
			...
			switch(Version)
			{
				...
				case 3:
					...
					yield return new Insert<Status>().Set(Status.StatusID, 0).Set(Status.Name, "Under construction");
					yield return new Insert<Status>().Set(Status.StatusID, 1).Set(Status.Name, "Operational");
					...
					yield return new Insert<Factory>().Set(Factory.PlanetID, planetID).Set(Factory.Name, "Stockpile").Set(Factory.StatusID, 1);
