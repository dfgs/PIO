...
namespace PIO.ServerLib.Modules
{
	public class TaskModule : DatabaseModule,ITaskModule
	{
		...
		public Task CreateTask(TaskTypeIDs TaskTypeID, int WorkerID, int? X, int? Y,int? BuildingID, int? TargetFactoryID, ResourceTypeIDs? ResourceTypeID, FactoryTypeIDs? FactoryTypeID, DateTime ETA)
		{
			...
			item=new Task() { TaskTypeID=TaskTypeID, WorkerID=WorkerID, X=X,Y=Y,BuildingID=BuildingID, FactoryID =TargetFactoryID, ResourceTypeID=ResourceTypeID, FactoryTypeID=FactoryTypeID, ETA=ETA };
			query = new Insert().Into(PIODB.TaskTable).Set(TaskTable.TaskTypeID,item.TaskTypeID).Set(TaskTable.WorkerID, item.WorkerID)
				...
				.Set(TaskTable.BuildingID, item.BuildingID).Set(TaskTable.FactoryID, item.FactoryID)
