...
namespace PIO.ServerLib.Modules
{
	public class TaskModule : DatabaseModule,ITaskModule
	{
		...
		public Task GetTask(int TaskID)
		{
			...
			query=new Select(TaskTable.TaskID, TaskTable.TaskTypeID, TaskTable.WorkerID, TaskTable.X, TaskTable.Y, TaskTable.BuildingID,  TaskTable.ResourceTypeID, TaskTable.BuildingTypeID,  TaskTable.ETA).From(PIODB.TaskTable).Where(TaskTable.TaskID.IsEqualTo(TaskID));
		...
		public Task GetLastTask(int WorkerID)
		{
			...
			query=new Select(TaskTable.TaskID, TaskTable.TaskTypeID, TaskTable.WorkerID, TaskTable.X, TaskTable.Y, TaskTable.BuildingID,  TaskTable.ResourceTypeID, TaskTable.BuildingTypeID,  TaskTable.ETA).Top(1).From(PIODB.TaskTable).Where(TaskTable.WorkerID.IsEqualTo(WorkerID)).OrderBy(OrderModes.DESC, TaskTable.TaskID);
			return TrySelectFirst<TaskTable, Task>(query).OrThrow<PIODataException>("Failed to query");
		}
		...
		public Task[] GetTasks(int WorkerID)
		{
			...
			Log(LogLevels.Information, $"Querying Task table (WorkerID={WorkerID})");
			query=new Select(TaskTable.TaskID, TaskTable.TaskTypeID, TaskTable.WorkerID, TaskTable.X, TaskTable.Y, TaskTable.BuildingID, TaskTable.ResourceTypeID, TaskTable.BuildingTypeID, TaskTable.ETA).From(PIODB.TaskTable).Where(TaskTable.WorkerID.IsEqualTo(WorkerID));
		...
		public Task[] GetTasks()
		{
			...
			query=new Select(TaskTable.TaskID, TaskTable.TaskTypeID, TaskTable.WorkerID, TaskTable.X, TaskTable.Y, TaskTable.BuildingID,  TaskTable.ResourceTypeID, TaskTable.BuildingTypeID,  TaskTable.ETA).From(PIODB.TaskTable);
			return TrySelectMany<TaskTable, Task>(query).OrThrow<PIODataException>("Failed to query");
		}
		...
		public Task CreateTask(TaskTypeIDs TaskTypeID, int WorkerID, int X, int Y,int? BuildingID, ResourceTypeIDs? ResourceTypeID, BuildingTypeIDs? BuildingTypeID, DateTime ETA)
		{
			...
			item = new Task() { TaskTypeID = TaskTypeID, WorkerID = WorkerID, X = X, Y = Y, BuildingID = BuildingID, ResourceTypeID = ResourceTypeID, BuildingTypeID = BuildingTypeID, ETA=ETA };
