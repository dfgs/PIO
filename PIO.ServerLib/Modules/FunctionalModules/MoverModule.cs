...
namespace PIO.ServerLib.Modules
{
	public class MoverModule : TaskGeneratorModule, IMoverModule
	{
		...
		public Task BeginMoveTo(int WorkerID, int X, int Y)
		{
			...
			task = Try(() => taskModule.CreateTask(TaskTypeIDs.MoveTo, WorkerID, X,Y, null, null, null, null, GetLastETA(WorkerID).AddSeconds(10))).OrThrow<PIOInternalErrorException>("Failed to create task");
